...
\begin{frame}{例題6　解答}
  ...
  \begin{enumerate}[1.]
    ...
    \item[] $= \int_{t}^0 f(u)g(t-u)(-du) = \int_{0}^t g(t-u)f(u)du$
    \item[] $= g(t)*f(t)$
  \end{enumerate}
  \end{exampleblock}
\end{frame}
...
\begin{frame}{演習問題（p.\,64--67）}
  ...
  \begin{description}
    ...
    \item[問7] たたみこみ定理を使い、 $L^{-1}[F(s)\cdot s/(s^{2}+\alpha^{2})]$ をたたみこみで表せ
...
\begin{frame}{問5　解答}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $(1) e^t * \sin t$ のたたみこみ
    \item $    L[e^t]\cdot L[\sin t] = 1/(s-1)\cdot1/(s^{2}+1) = 1/((s-1)(s^{2}+1))$
    \item 部分分数 $: A/(s-1) + (Bs+C)/(s^{2}+1)$
    \item $A = 1/2; Bs+C$ の係数 $: B = -1/2, C = -1/2$
    \item $    L^{-1} = (e^t - \cos t - \sin t)/2$
    \item (2) t * cos t のたたみこみ
    \item $    L[t]\cdot L[\cos t] = 1/s^{2}\cdot s/(s^{2}+1) = 1/(s(s^{2}+1))$
    \item 部分分数 $: A/s + (Bs+C)/(s^{2}+1): A=1, B=-1, C=0$
    \item $    L^{-1} = 1 - \cos t$
  \end{itemize}
  \end{block}
...
\begin{frame}{問6　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 定義 $: f*(g_{1}+g_{2}) = \int_{0}^t f(t-\tau)(g_{1}(\tau)+g_{2}(\tau))d\tau$
    \item 積分の線形性より $= \int_{0}^t f(t-\tau)g_{1}(\tau)d\tau + \int_{0}^t f(t-\tau)g_{2}(\tau)d\tau$
    \item $= f*g_{1} + f*g_{2}$（証明終）
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{問7　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $L[\cos(\alpha t)] = s/(s^{2}+\alpha^{2})$
    \item たたみこみ定理 $: L^{-1}[F(s)\cdot G(s)] = f(t)*g(t)$
    \item $G(s) = s/(s^{2}+\alpha^{2})  \to  g(t) = \cos(\alpha t)$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    L^{-1}[F(s)\cdot s/(s^{2}+\alpha^{2})] = f(t)*\cos(\alpha t) = \int_{0}^t f(t-\tau)\cos(\alpha\tau)d\tau
  \]
  \end{exampleblock}
  {\footnotesize \textcolor{gray}{\textit{元 YAML の問題文「 $L^{-1}[\cos(\alpha t)]$ のたたみこみ表現」は教科書で要確認}}}
\end{frame}

\begin{frame}{問8　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $(1) L^{-1}[F(s)\cdot1/(s-\alpha)^{2}]:  L^{-1}[1/(s-\alpha)^{2}] = te^{\alpha t}$
    \item たたみこみ定理より $f(t)*te^{\alpha t} = \int_{0}^t f(t-\tau)\cdot\tau\cdot e^{\alpha\tau}d\tau$
    \item $(2) L^{-1}[F(s)\cdot1/((s-\alpha)(s-\beta))]:  L^{-1}[1/((s-\alpha)(s-\beta))] = (e^{\alpha t}-e^{\beta t})/(\alpha-\beta)$
    \item $    = f(t)*(e^{\alpha t}-e^{\beta t})/(\alpha-\beta) = (1/(\alpha-\beta))\int_{0}^t f(t-\tau)(e^{\alpha\tau}-e^{\beta\tau})d\tau$
    \item $(3) L^{-1}[F(s)\cdot1/((s-\alpha)^{2}+\beta^{2})]:  L^{-1}[1/((s-\alpha)^{2}+\beta^{2})] = (1/\beta)e^{\alpha t}\sin(\beta t)$
    \item $    = f(t)*(1/\beta)e^{\alpha t}\sin(\beta t) = (1/\beta)\int_{0}^t f(t-\tau)e^{\alpha\tau}\sin(\beta\tau)d\tau$
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{問9　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $\int_{0}^t x(t-\tau)e^\tau d\tau = x(t)*e^t$ のたたみこみ
    \item 両辺をラプラス変換 $: X(s) + X(s)\cdot1/(s-1) = s/(s^{2}+9)$
    \item $X(s)(1 + 1/(s-1)) = s/(s^{2}+9)$
    \item $X(s)\cdot s/(s-1) = s/(s^{2}+9)$
    \item $X(s) = (s-1)/(s^{2}+9) = s/(s^{2}+9) - 1/(s^{2}+9)$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    x(t) = \cos(3t) - (1/3)\sin(3t)
  \]
  \end{exampleblock}
\end{frame}
