...
\begin{frame}{問14　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 両辺をラプラス変換 $: (sF(s)-f(0)) + 2F(s) = 1/s^{2}$
    \item $f(0)=0$ なので $(s+2)F(s) = 1/s^{2}$
    \item $F(s) = 1/(s^{2}(s+2))$
    \item 部分分数 $: 1/(s^{2}(s+2)) = A/s + B/s^{2} + C/(s+2)$
    \item $A = -1/4,  B = 1/2,  C = 1/4$
    \item $F(s) = -1/(4s) + 1/(2s^{2}) + 1/(4(s+2))$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    f(t) = -1/4 + t/2 + (1/4)e^{-2t}
  \]
  \end{exampleblock}
\end{frame}
