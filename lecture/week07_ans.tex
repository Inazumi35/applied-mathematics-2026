...
\begin{frame}{問11　解答}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item ラプラス変換を使った証明:
    \item $L[f(t)*\delta(t)] = F(s)\cdot L[\delta(t)] = F(s)\cdot1 = F(s) = L[f(t)]$
    \item $\therefore f(t)*\delta(t) = f(t)$
    \item 交換法則より $\delta(t)*f(t) = f(t)*\delta(t) = f(t)$ （証明終）
    \item （直接証明） $f(t)*\delta(t) = \int_{0}^t f(t-\tau)\delta(\tau)d\tau = f(t-0) = f(t)$
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{問12　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 両辺をラプラス変換（初期値ゼロ）:
    \item $(s^{2} + \omega^{2})Y(s) = L[\delta(t)] = 1$
    \item $Y(s) = 1/(s^{2}+\omega^{2}) = (1/\omega)\cdot\omega/(s^{2}+\omega^{2})$
    \item 逆ラプラス変換 $: y(t) = (1/\omega)\sin(\omega t)$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    y(t) = (1/\omega)\sin(\omega t)  （デルタ入力に対するインパルス応答）
  \]
  \end{exampleblock}
  {\footnotesize \textcolor{gray}{\textit{p.70 スキャンで確認（元YAMLの問題文は誤記）}}}
\end{frame}
