...
\begin{frame}{問3　解答}
  $(1) l=1: a_{0}=1, a_{n}=\int_{-1}^{0}\cos(n\pi x)dx=0, b_{n}=\int_{-1}^{0}\sin(n\pi x)dx=((-1)^n-1)/(n\pi)$\\[4pt]
  $(2) l=2:$ 平均 $= 3, a_{n}=0, b_{n}=(1/2)[2\int_{-2}^{0}\sin(n\pi x/2)dx + 4\int_{0}^{2}\sin(n\pi x/2)dx]$\\[4pt]
  $    b_{n} = 2(1-(-1)^n)/(n\pi)$\\[4pt]
  $(3) l=1/2: a_{0}=(1/2), a_{n}=(1-(-1)^n)/(n^{2}\pi^{2}), b_{n}=-1/(n\pi)$
  \medskip\textbf{答}：
