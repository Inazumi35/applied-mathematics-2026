...
\begin{frame}{例題7　解答}
  ...
  \medskip\textbf{答}：$L[\sin(\omega t)] = \omega/(s^{2}+\omega^{2})$（$\omega$ の正負に関係なく）
\end{frame}
...
\begin{frame}{問8　解答}
  ...
  \medskip\textbf{答}：$L[\cos(\omega t)] = s/(s^{2}+\omega^{2})  (s > 0)$
\end{frame}

\begin{frame}{問9　解答}
  $\cos^{2}t = (1+\cos 2t)/2   \to   L[\cos^{2}t] = (1/2)(1/s + s/(s^{2}+4)) = (s^{2}+2)/(s(s^{2}+4))$\\[4pt]
  $\sin^{2}t = (1-\cos 2t)/2   \to   L[\sin^{2}t] = (1/2)(1/s - s/(s^{2}+4)) = 2/(s(s^{2}+4))$\\[4pt]
  $\sin t \cos t = (1/2)\sin 2t   \to   L[\sin t \cos t] = (1/2)\cdot2/(s^{2}+4)$
  \medskip\textbf{答}：
...
\begin{frame}{問10　解答}
  $L[\cos(\mu t)] = s/(s^{2}+\mu^{2})$\\[4pt]
  像関数の移動法則 $: L[e^{\mu t}f(t)] = F(s-\mu)$ より $s \to s-\mu$ に置き換える
  \medskip\textbf{答}：$L[e^{\mu t}\cos(\mu t)] = (s-\mu)/((s-\mu)^{2}+\mu^{2})$
\end{frame}

\begin{frame}{問11　解答}
  原関数の移動法則 $: L[f(t-\mu)U(t-\mu)] = e^{-\mu s}F(s)$\\[4pt]
  ここで $f(t) = \sin t, \mu = \pi/4, F(s) = L[\sin t] = 1/(s^{2}+1)$
  \medskip\textbf{答}：$L[\sin(t-\pi/4)\cdot U(t-\pi/4)] = e^{-\pi s/4}/(s^{2}+1)$
...
\begin{frame}{問12　解答}
  ...
  \medskip\textbf{答}：$L[f(t)] = e^{-2s}/s^{2}  (s > 0)$
\end{frame}
