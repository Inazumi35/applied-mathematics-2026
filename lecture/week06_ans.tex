...
\begin{frame}{問7　解答}
  【方法1：たたみこみを直接計算】\\[4pt]
  $t^{2}*t = \int_{0}^t \tau^{2}(t-\tau) d\tau = t\cdot[\tau^{3}/3]_{0}^t - [\tau⁴/4]_{0}^t = t⁴/3 - t⁴/4 = t⁴/12$\\[4pt]
  $L[t⁴/12] = (1/12)\cdot4!/s⁵ = (1/12)\cdot24/s⁵$\\[4pt]
  【方法2：問5（たたみこみ定理）を使う】\\[4pt]
  $L[t^{2}*t] = L[t^{2}]\cdot L[t] = (2/s^{3})\cdot(1/s^{2})$
  \medskip\textbf{答}：$L[t^{2}*t] = 2/s⁵$
\end{frame}
...
\begin{frame}{問9　解答}
  ...
  \medskip\textbf{答}：$x(t) = \cos(3t) - (1/3)\sin(3t)$
\end{frame}
