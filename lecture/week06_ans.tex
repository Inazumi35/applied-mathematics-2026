...
\begin{frame}{例題5　解答}
  ...
  \\[8pt]\textcolor{red}{\textbf{答}}：$\int_{0}^t \sin \tau \cdot \cos(t-\tau) d\tau = (t/2) \sin t$
\end{frame}
...
\begin{frame}{問5　解答}
  $(1) e^t * \sin t$ のたたみこみ\\[4pt]
  $    L[e^t]\cdot L[\sin t] = 1/(s-1)\cdot1/(s^{2}+1) = 1/((s-1)(s^{2}+1))$\\[4pt]
  部分分数 $: A/(s-1) + (Bs+C)/(s^{2}+1)$\\[4pt]
  $A = 1/2; Bs+C$ の係数 $: B = -1/2, C = -1/2$\\[4pt]
  $    L^{-1} = (e^t - \cos t - \sin t)/2$\\[4pt]
  $(2) t * \cos t$ のたたみこみ\\[4pt]
  $    L[t]\cdot L[\cos t] = 1/s^{2}\cdot s/(s^{2}+1) = 1/(s(s^{2}+1))$\\[4pt]
  部分分数 $: A/s + (Bs+C)/(s^{2}+1): A=1, B=-1, C=0$\\[4pt]
  $    L^{-1} = 1 - \cos t$
...
\begin{frame}{問6　解答}
  定義 $: f*(g_{1}+g_{2}) = \int_{0}^t f(t-\tau)(g_{1}(\tau)+g_{2}(\tau))d\tau$\\[4pt]
  積分の線形性より $= \int_{0}^t f(t-\tau)g_{1}(\tau)d\tau + \int_{0}^t f(t-\tau)g_{2}(\tau)d\tau$\\[4pt]
  $= f*g_{1} + f*g_{2}$（証明終）
\end{frame}
...
\begin{frame}{問7　解答}
  【方法1：たたみこみを直接計算】\\[4pt]
  $t^{2}*t = \int_{0}^t \tau^{2}(t-\tau) d\tau = t\cdot[\tau^{3}/3]_{0}^t - [\tau^{4}/4]_{0}^t = t^{4}/3 - t^{4}/4 = t^{4}/12$\\[4pt]
  $L[t^{4}/12] = (1/12)\cdot4!/s^{5} = (1/12)\cdot24/s^{5}$\\[4pt]
  【方法2：問5（たたみこみ定理）を使う】\\[4pt]
...
\begin{frame}{問8　解答}
  ...
  たたみこみ定理より $f(t)*te^{\alpha t} = \int_{0}^t f(t-\tau)\cdot\tau\cdot e^{\alpha\tau}d\tau$\\[4pt]
  $(2) L^{-1}[F(s)\cdot1/((s-\alpha)(s-\beta))]:  L^{-1}[1/((s-\alpha)(s-\beta))] = (e^{\alpha t}-e^{\beta t})/(\alpha-\beta)$\\[4pt]
  $    = f(t)*(e^{\alpha t}-e^{\beta t})/(\alpha-\beta) = (1/(\alpha-\beta))\int_{0}^t f(t-\tau)(e^{\alpha\tau}-e^{\beta\tau})d\tau$\\[4pt]
  $(3) L^{-1}[F(s)\cdot1/((s-\alpha)^{2}+\beta^{2})]:  L^{-1}[1/((s-\alpha)^{2}+\beta^{2})] = (1/\beta)e^{\alpha t}\sin(\beta t)$\\[4pt]
  $    = f(t)*(1/\beta)e^{\alpha t}\sin(\beta t) = (1/\beta)\int_{0}^t f(t-\tau)e^{\alpha\tau}\sin(\beta\tau)d\tau$
\end{frame}
...
\begin{frame}{問9　解答}
  $\int_{0}^t x(t-\tau)e^\tau d\tau = x(t)*e^t$ のたたみこみ\\[4pt]
