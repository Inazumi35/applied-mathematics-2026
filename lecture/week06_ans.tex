...
\begin{frame}{問7　解答}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 【方法1：たたみこみを直接計算】
    \item $t^{2}*t = \int_{0}^t \tau^{2}(t-\tau) d\tau = t\cdot[\tau^{3}/3]_{0}^t - [\tau⁴/4]_{0}^t = t⁴/3 - t⁴/4 = t⁴/12$
    \item $L[t⁴/12] = (1/12)\cdot4!/s⁵ = (1/12)\cdot24/s⁵$
    \item 【方法2：問5（たたみこみ定理）を使う】
    \item $L[t^{2}*t] = L[t^{2}]\cdot L[t] = (2/s^{3})\cdot(1/s^{2})$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
