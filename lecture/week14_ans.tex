...
\begin{frame}{問10　解答}
  ...
  $a_{0} = 2\int_{0}^1(1-x)dx = 1$\\[4pt]
  $a_{n} = 2\int_{0}^1(1-x)\cos(n\pi x)dx = 2(1-(-1)^n)/(n^{2}\pi^{2})$\\[4pt]
  n が奇数 $: a_{n} = 4/(n^{2}\pi^{2}), n$ が偶数 $: a_{n} = 0$\\[4pt]
  複素フーリエ係数 $: c_{0} = a_{0}/2 = 1/2, |c_{n}| = a_{n}/2 = 2/(n^{2}\pi^{2})$ （奇数 n ）
  \medskip\textbf{答}：スペクトル $: |c_{0}|=1/2, |c_{n}|=2/(n^{2}\pi^{2})$ （ $n=1,3,5,...$ ） , 0 （ $n=2,4,6,...$ ）
\end{frame}

\begin{frame}{問11　解答}
  非周期関数 $ \to $ フーリエ変換でスペクトルを求める\\[4pt]
  $f(x)$ は偶関数 $: F(u) = 2\int_{0}^1(1-x)\cos(ux)dx$\\[4pt]
  $= 2{[(1-x)\sin(ux)/u]_{0}^1 + (1/u)\int_{0}^1 \sin(ux)dx}$\\[4pt]
  $= 2(1/u)[-\cos(ux)/u]_{0}^1 = 2(1-\cos u)/u^{2} = 4\sin^{2}(u/2)/u^{2}$
  \medskip\textbf{答}：スペクトル $S(u) = |F(u)|/\pi = 4\sin^{2}(u/2)/(\pi u^{2})$ （連続スペクトル）
\end{frame}
