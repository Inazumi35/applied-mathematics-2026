...
\begin{frame}{問10　解答}
  ...
  $a_{n} = 2\displaystyle\int_{0}^1(1-x)\cos(n\pi x)\,dx = \dfrac{2(1-(-1)^n)}{n^{2}\pi^{2}}$\\[6pt]
  $n$ が奇数 $:\; a_{n} = \dfrac{4}{n^{2}\pi^{2}},\quad n$ が偶数 $:\; a_{n} = 0$\\[6pt]
  複素フーリエ係数 $:\; |c_{n}| = \dfrac{a_{n}}{2} = \dfrac{2}{n^{2}\pi^{2}}$（奇数 $n$）
  \\[8pt]\textcolor{red}{\textbf{答}}：スペクトル $:\; |c_{0}|=\dfrac{1}{2},\; |c_{n}|=\dfrac{2}{n^{2}\pi^{2}}$（$n=1,3,5,\ldots$），$0$（$n=2,4,6,\ldots$）
\end{frame}
...
\begin{frame}{問11　解答}
  ...
  $f(x)$ は偶関数 $:\; F(u) = 2\displaystyle\int_{0}^1(1-x)\cos(ux)\,dx$\\[4pt]
  $= 2\!\left\{\!\left[\dfrac{(1-x)\sin(ux)}{u}\right]_{0}^1 + \dfrac{1}{u}\displaystyle\int_{0}^1 \sin(ux)\,dx\right\}$\\[4pt]
  $= \dfrac{2}{u}\!\left[-\dfrac{\cos(ux)}{u}\right]_{0}^1 = \dfrac{2(1-\cos u)}{u^{2}} = \dfrac{4\sin^{2}(u/2)}{u^{2}}$
  \\[8pt]\textcolor{red}{\textbf{答}}：スペクトル $S(u) = \dfrac{|F(u)|}{\pi} = \dfrac{4\sin^{2}(u/2)}{\pi u^{2}}$（連続スペクトル）
\end{frame}
