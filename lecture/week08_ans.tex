...
\begin{frame}{定義：フーリエ級数（周期 $2\pi$）}
  \begin{block}{}
  $f(x)$ が周期 $2\pi$ の区分的に滑らかな関数であるとき、\\[2pt]
  次の無限級数に展開できる（フーリエ級数という）\\[2pt]
  \[
...
\begin{frame}{問2　解答（1/3）$c_{0},\,a_{n}$ の計算}
  $f(x) = x$ は奇関数．\\[4pt]
  $c_{0} = \dfrac{1}{2\pi}\displaystyle\int_{-\pi}^{\pi} x\,dx = 0$
  \quad（奇関数の対称区間積分）\\[6pt]
  $a_{n} = \dfrac{1}{\pi}\displaystyle\int_{-\pi}^{\pi} x\cos(nx)\,dx = 0$
  \quad（$x\cos(nx)$：奇$\times$偶$=$奇）\\[6pt]
  $b_{n} \neq 0$\quad（$x\sin(nx)$：奇$\times$奇$=$偶）
\end{frame}

\begin{frame}{問2　解答（2/3）$b_{n}$ の計算}
  \begin{align*}
...
\begin{frame}{問2　解答（3/3）$b_{n}$ の続き}
