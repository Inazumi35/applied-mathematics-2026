...
\begin{frame}{問8　解答}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 相似性 $: L[f(at)] = (1/a)F(s/a), F(s) = L[\cos t] = s/(s^{2}+1)$ を利用
    \item $L[\cos(\omega t)] = (1/\omega)\cdot(s/\omega)/((s/\omega)^{2}+1) = (1/\omega)\cdot(s/\omega)\cdot\omega^{2}/(s^{2}+\omega^{2})$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    L[\cos(\omega t)] = s/(s^{2}+\omega^{2})  (s > 0)
  \]
  \end{exampleblock}
\end{frame}
...
\begin{frame}{問9　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $\cos^{2}t = (1+\cos 2t)/2   \to   L[\cos^{2}t] = (1/2)(1/s + s/(s^{2}+4)) = (s^{2}+2)/(s(s^{2}+4))$
    \item $\sin^{2}t = (1-\cos 2t)/2   \to   L[\sin^{2}t] = (1/2)(1/s - s/(s^{2}+4)) = 2/(s(s^{2}+4))$
    \item $\sin t \cos t = (1/2)\sin 2t   \to   L[\sin t \cos t] = (1/2)\cdot2/(s^{2}+4)$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
...
\begin{frame}{問10　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $L[\cos(\mu t)] = s/(s^{2}+\mu^{2})$
    \item 像関数の移動法則 $: L[e^{\mu t}f(t)] = F(s-\mu)$ より $s \to s-\mu$ に置き換える
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    L[e^{\mu t}\cos(\mu t)] = (s-\mu)/((s-\mu)^{2}+\mu^{2})
  \]
  \end{exampleblock}
\end{frame}
...
\begin{frame}{問11　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item 原関数の移動法則 $: L[f(t-\mu)U(t-\mu)] = e^{-\mu s}F(s)$
    \item ここで $f(t) = \sin t, \mu = \pi/4, F(s) = L[\sin t] = 1/(s^{2}+1)$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
    L[\sin(t-\pi/4)\cdot U(t-\pi/4)] = e^{-\pi s/4}/(s^{2}+1)
  \]
  \end{exampleblock}
  {\footnotesize \textcolor{gray}{\textit{YAML に $L^{-1}$ とあるが正しくは L （ラプラス変換）}}}
\end{frame}

\begin{frame}{問12　解答}
  \begin{block}{}
  \textbf{グラフ：}t < 2 では $f(t)=0, t≧2$ では原点を $t=2$ に平行移動した直線（傾き 1 ）\\[0.3em]
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $f(t) = g(t-2)U(t-2)$ と見る（ $g(t) = t$ ）
    \item 原関数の移動法則 $: L[g(t-2)U(t-2)] = e^{-2s}\cdot L[t] = e^{-2s}/s^{2}$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
