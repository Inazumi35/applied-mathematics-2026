...
\begin{frame}{問1　解答}
  ...
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $(1) F(u) = \int_{-\infty}^{0} e^x e^{-iux} dx = \int_{-\infty}^{0} e^{(1-iu)x} dx$
    \item $    = [e^{(1-iu)x}/(1-iu)]_{-\infty}^{0} = 1/(1-iu)  (\Re(1-iu)=1>0)$
    \item $(2) G(u) = \int_{0}^{\infty} x e^{-x} e^{-iux} dx = \int_{0}^{\infty} x e^{-(1+iu)x} dx$
    \item $\int_{0}^\infty x e^{-ax} dx = 1/a^{2}$ より $G(u) = 1/(1+iu)^{2}$
  \end{itemize}
  \end{block}
...
\begin{frame}{問2　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $(1) F(u) = \int_{0}^1 e^{-iux} dx = [e^{-iux}/(-iu)]_{0}^1 = (1-e^{-iu})/(iu)$
    \item $(2) g(x)$ は奇関数 $: G(u) = \int_{-1}^{1} x e^{-iux} dx = -2i\int_{0}^1 x \sin(ux) dx$
    \item $    \int_{0}^1 x \sin(ux) dx = [-x \cos(ux)/u]_{0}^1 + (1/u)\int_{0}^1 \cos(ux)dx$
    \item $    = -\cos u/u + \sin u/u^{2}$
    \item $    G(u) = -2i(-\cos u/u + \sin u/u^{2}) = 2i(\cos u/u - \sin u/u^{2})$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
  $(1) F(u) = (1-e^{-iu})/(iu)$\\
...
\begin{frame}{問3　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item (1) フーリエ積分定理 $: f(x) = (1/2\pi)\int F(u)e^{iux}du$
    \item 連続点では $f(x)$ 、不連続点 $x=0,1$ では $(f(x+0)+f(x-0))/2 = 1/2 ✓$
    \item $(2) x=1/2$ （連続点）を代入 $: f(1/2) = 1 = (1/2\pi)\int(1-e^{-iu})/(iu)\cdot e^{iu/2}du$
    \item 実部を取ると $: 1 = (1/\pi)\int_{0}^\infty \sin u/u du$
    \item $     \to  \int_{-\infty}^{\infty} \sin u/u du = \pi ✓$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
...
\begin{frame}{問4　解答}
  \begin{block}{}
  \begin{itemize}\setlength{\itemsep}{2pt}
    \item $f(x)$ は奇関数なのでフーリエサイン変換を用いる
    \item $Fs(u) = 2\int_{0}^1 \sin(ux) dx = 2[-\cos(ux)/u]_{0}^1 = 2(1-\cos u)/u$
  \end{itemize}
  \end{block}
  \begin{exampleblock}{答え}
    \[
