...
\begin{frame}{例題3　解答}
  ...
  $F(u) = 2\int_{0}^{1} (1-x) \cos(ux) dx$\\[4pt]
  $= 2\begin{cases} [(1-x)\sin(ux)/u]_{0}^1 + (1/u)\int_{0}^1 \sin(ux) dx \end{cases}$\\[4pt]
  $= 2\begin{cases} 0 + (1/u)[-\cos(ux)/u]_{0}^1 \end{cases} = 2(1-\cos u)/u^{2}$
  \medskip\textbf{答}：$F(u) = 2(1-\cos u)/u^{2}$
\end{frame}
...
\begin{frame}{問3　解答}
  ...
  $(2) x=1/2$ （連続点）を代入 $: f(1/2) = 1 = (1/2\pi)\int(1-e^{-iu})/(iu)\cdot e^{iu/2}du$\\[4pt]
  実部を取ると $: 1 = (1/\pi)\int_{0}^\infty \sin u/u du$\\[4pt]
...
\begin{frame}{問4　解答}
  ...
  $Fs(u) = 2\int_{0}^1 \sin(ux) dx = 2[-\cos(ux)/u]_{0}^1 = 2(1-\cos u)/u$
  \medskip\textbf{答}：$Fs(u) = 2(1-\cos u)/u$
\end{frame}
